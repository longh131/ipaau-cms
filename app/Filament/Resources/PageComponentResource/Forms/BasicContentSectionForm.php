...
class BasicContentSectionForm
{
    ...
    public static function schema(
        ...
        string $sectionTitle,
        string $titleHeadingTag = 'H1',
        int $maxButtons = 6,
    ): array {
        return [
            Section::make($sectionTitle)
                ->visible(fn (Get $get): bool => $get('component_type') === $componentType)
                ->statePath('data')
                ->schema([
                    Forms\Components\TextInput::make('tagline')
                        ->label('小标题')
                        ->placeholder('例如：YOUR MEMBERSHIP')
                        ->maxLength(255)
                        ->columnSpanFull(),
                    Forms\Components\Repeater::make('title_lines')
                        ->label('大标题')
                        ->helperText("每行对应前台一个 {$titleHeadingTag}，可添加多行")
                        ->schema([

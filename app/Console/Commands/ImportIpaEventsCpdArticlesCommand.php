...
class ImportIpaEventsCpdArticlesCommand extends Command
{
    protected $signature = 'articles:import-ipa-events-cpd
                            {--file=bak/IPA_news.xlsx : Excel 文件路径（相对项目根目录）}
                            {--category=69 : 目标栏目 ID}
                            {--slug-prefix=events-cpd : 别名前缀}
                            {--dry-run : 仅预览，不写入数据库}';

    protected $description = '从 IPA_news.xlsx 导入含【】标题的活动&CPD预告文章';
    ...
    public function handle(): int
    {
        ...
        if ($dryRun) {
            $this->info("预览完成：可导入 {$created} 条，跳过空标题 {$skipped} 条，跳过非【】标题 {$skippedNoBrackets} 条，图片链接替换 {$imageReplacements} 处。");
        } else {
            $this->info("导入完成：新增 {$created} 条，更新 {$updated} 条，跳过空标题 {$skipped} 条，跳过非【】标题 {$skippedNoBrackets} 条，图片链接替换 {$imageReplacements} 处。");
